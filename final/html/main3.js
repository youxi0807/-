let sum3 = 0;
let count_sum3=0;
let timesum3 = 0;
var m3=['海鮮義大利麵','豬里肌義大利麵','雞柳義大利麵','海鮮風味燉飯','原味什錦鍋','原味鍋燒麵','脆皮雞排','木瓜牛奶'];
var meal3 = new Vue({
    el:'#app',
    data:{
        itemList:[
          {
            id:'1',
            itemName:'海鮮義大利麵',
            imgUrl:'https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcRYR9ws1MrhlTMJFaHWytU5y9R_Eq10eYagS4VFfQTtZdz6q29-',
            price:'120',
            count:'0',
            time:'25'
          },
          {
            id:'2',
            itemName:'豬里肌義大利麵',
            imgUrl:'https://encrypted-tbn2.gstatic.com/images?q=tbn:ANd9GcQUlw8YYdSfoBRL8AUuxvt4BqtqnDUfYZ98_JfN1otCXrwAMGsc',
            price:'110',
            count:'0',
            time:'20'
          },
          {
            id:'3',
            itemName:'雞柳義大利麵',
            imgUrl:'https://encrypted-tbn2.gstatic.com/images?q=tbn:ANd9GcQUlw8YYdSfoBRL8AUuxvt4BqtqnDUfYZ98_JfN1otCXrwAMGsc',
            price:'130',
            count:'0',
            time:'20'
          },
          {
            id:'4',
            itemName:'海鮮風味燉飯',
            imgUrl:'https://d3l76hx23vw40a.cloudfront.net/recipe/yqn31-018.jpg',
            price:'130',
            count:'0',
            time:'25'
          },
          {
            id:'5',
            itemName:'原味什錦鍋',
            imgUrl:'http://www.ttv.com.tw/recipe/images/%E5%9C%98%E5%9C%93%E5%B9%B4%E8%8F%9C%E5%81%A5%E5%BA%B7%E9%A4%90/335/%E4%BB%80%E9%8C%A6%E7%81%AB%E9%8D%8B.jpg',
            price:'120',
            count:'0',
            time:'15'
          }, 
          {
            id:'6',
            itemName:'原味鍋燒麵',
            imgUrl:'https://www.unileverfoodsolutions.tw/dam/global-ufs/mcos/na/taiwan/calcmenu/recipes/TW-recipes/general/%E6%B5%B7%E9%AE%AE%E9%8D%8B%E7%87%92%E9%BA%B5/main-header.jpg',
            price:'70',
            count:'0',
            time:'15'

          }, 
          {
            id:'7',
            itemName:'脆皮雞排',
            imgUrl:'https://d3l76hx23vw40a.cloudfront.net/recipe/bk145-016.jpg',
            price:'55',
            count:'0',
            time:'10'
          }, 
          {
            id:'8',
            itemName:'木瓜牛奶',
            imgUrl:'https://d3l76hx23vw40a.cloudfront.net/recipe/gy49-056.jpg',
            price:'60',
            count:'0',
            time:'2'
          }, 
    ]
    },
    methods:{
      handlePlus: function(item){
        item.count++;
        sum3 += parseInt(item.price);
        document.getElementById("price").innerHTML = sum3;
        count_sum3 ++;
        document.getElementById("count").innerHTML = count_sum3;
        timesum3 += parseInt(item.time);
        document.getElementById("time").innerHTML = timesum3;
      },
      handleSub: function(item){
        if(item.count>0){
          item.count--;
          sum3 -= parseInt(item.price);
          document.getElementById("price").innerHTML = sum3;
          count_sum3 --;
          document.getElementById("count").innerHTML = count_sum3;
          timesum3 -= parseInt(item.time);
          document.getElementById("time").innerHTML = timesum3;               
        }
      },
        handledelete: function(index){
            console.log(this);
            this.itemList.splice(index,1);
        },
    },
})
function ShowMsgFromScript3(){
  let flag = 0, book = new Array(0, 0, 0, 0, 0, 0, 0, 0);
  while (!flag)
  {
    var i=Math.floor(Math.random()*8);
    let hungry = $("#typ").val();
    console.log(i);
    if (hungry == 10 && parseInt(meal3.itemList[i].time) <= 15)
    {
      document.getElementById("meal").innerHTML=m3[i];
      flag = 1;
    }
    else if (hungry <=9 && hungry>= 5 && parseInt(meal3.itemList[i].time) <= 20)
    {
      $("#meal").html(m3[i]);
      flag = 1;
    }
    else if (hungry <= 4 && hungry>= 0 && parseInt(meal3.itemList[i].time) <= 40)
    {
      $("#meal").html(m3[i]);
      flag = 1;
    }
    else
    {
      book[i] = 1;
    }
    let temp = 1;
    for (let i = 0; i < 8; i++)
    {
      if (book[i] == 0){
        temp = 0;
      }
    }
    if (temp == 1)
    {
      $("#meal").html("沒有適合你的餐點!");
      flag = 1;
    }
  }
}